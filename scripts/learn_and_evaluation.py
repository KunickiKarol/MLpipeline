...
def get_not_text(df):
    df_not_text = df.filter(
        regex="^origin_|^overall|^vote|^numbers_amount|^verified|^reviewTextLength|^unixReviewTime"
    )
...
if method == "cross":
    ...
    for model_idx, model in enumerate([dummy, RF_model, SVM_model]):
        ...
        for df_name, df_cross in zip(df_names, [df_train, df_text_only, df_not_text]):
            result = cross_validate(
                model,
                df_cross.drop(class_column_name, axis=1),
                df_cross[class_column_name],
                cv=5,
                scoring=["f1_micro"],
            )

            mlflow.log_metric(
                df_name + "_test_f1_micro", np.mean(result["test_f1_micro"])
            )
            results.append(
                {
                    type(model).__name__
                    + "_"
                    + df_name
                    + "_test_f1_micro": np.mean(result["test_f1_micro"])
                }
            )
    ...
    x_train, x_test = selection(
        x_train, x_test, y_train, selection_method, reduction_method
    )
